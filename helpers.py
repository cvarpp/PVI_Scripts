...
def query_dscf(sid_list=None, no_pbmcs=set(), use_cache=False, update_cache=False):
    ...
    if use_cache and os.path.exists('local_cache/dscf.h5') and '/proc_info' in pd.HDFStore('local_cache/dscf.h5', mode='r').keys():
        ...
        with warnings.catch_warnings():
            ...
            proc_nb_archive = pd.read_excel(util.proc + 'Processing Notebook Archive.xlsx', sheet_name='Specimen Dashboard', header=1).rename(columns=correct_new)
        dataframe_list = [bsl2p_archive, bsl2_archive, bsl2p_samples, bsl2_samples, crp_samples, proc_nb_archive, new_samples]
