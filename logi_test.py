...
def filter_box(box_name, uploaded, args, box_df=None):
    ...
    if box_df is None:
        return False
    return (box_name not in uploaded) and ((args.min_count <= box_df.set_index('Name').loc[box_name, 'Tube Count'] <= 81) or (box_name in full_des) or (box_df.set_index('Name').loc[box_name, 'Box Done?'] == 1.0))
