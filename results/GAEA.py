...
def gaea_report(args):
    gaea_data = pd.read_excel(util.gaea_folder + 'GAEA for D4 Long.xlsx', sheet_name=None)
    ppl_info = gaea_data['Baseline Info']
    ...
    gaea_ppl = set(ppl_info['Participant ID'].str.strip().unique()) - exclude_ppl
    intake = query_intake(participants=gaea_ppl)
    ...
    vax_data = gaea_data['COVID Vaccinations']
    vax_date_lkp = {}
    vax_dose_lkp = {}
    vax_type_lkp = {}
    for pid in gaea_ppl:
    ...
    cov_data = gaea_data['COVID Infections']
    cov_date_lkp = {}
    cov_test_lkp = {}
    for pid in gaea_ppl:
        cov_date_lkp[pid] = []
    ...
    # cancer_spec = gaea_data['Cancer-specific'].set_index('Participant ID').loc[:, cancer_cols]
    ...
    vax_df_cols = ['Date Collected', 'participant_id'] + ab_cols + ['Days to Most Recent Vaccine', 'Vax Count Pre-Sample', 'COVID Count Pre-Sample', 'Most Recent Vaccine Date', 'Most Recent Vaccine Type', 'Most Recent Vaccine Dose', 'Most Recent Infection Date', 'Most Recent Infection Test', 'Days to Most Recent Infection', 'Days to Dose 1', 'Days to Final Primary Dose'] + dem_cols
    vax_dfs = {}
    ...
    if not args.debug:
        with pd.ExcelWriter(util.gaea_folder + 'GAEA Central Info.xlsx') as writer:
...
if __name__ == '__main__':
    if len(sys.argv) != 1:
        argparser = argparse.ArgumentParser(description='Generate report for all GAEA samples')
