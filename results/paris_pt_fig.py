...
def main(pid, output_dir, display):
    with warnings.catch_warnings():
        ...
        #If ppl have both Kantaro and Abbott Results
        if (pd.to_datetime(results['Date Collected'].values[-1]) >= pd.to_datetime('1.18.23')) and cov22_length != 0:
            ...
            fig, axes = plt.subplots(2, 2, figsize=(10, 6), dpi=300, layout='tight', width_ratios=[covqa_proportion, cov22_proportion], height_ratios=[0.85, 0.15])
            ...
            def plot_raster(ax, dates, color):
                ...
                for date in dates:
                    if pd.to_datetime(date) > pd.to_datetime('1.1.2020'):
                            ax.axvline(x=pd.to_datetime(date, errors='coerce'), color=color, lw=3, linestyle='--')
                plt.xlim(x_limits)
            ...
            plt.yticks(yticks, yticklabels, fontsize=11)
            qalower, qaupper = axes[0,0].get_ylim()
            ...
            ax.set_xticks([])
            ax.set_yticks([])
            fig.text(.5,.075, 'This figure has been generated by the Personalized Virology Initiative using SARS-CoV-2 \n binding antibody results from FDA approved tests run in a CLIA-certified lab. \n Thank you for your support of COVID-19 research!',
                     horizontalalignment='center', verticalalignment='center', fontsize=11)
            border = plt.Rectangle(xy=(.005,.15), width=0.989, height=0.845, fill=False, edgecolor="#0a5279", lw=4, zorder=10)
            fig.add_artist(border)

            logo = Image.open(util.cross_project + '/Participant Plots/simon_lab_logo.png')
            logo = logo.resize((278, 208))
            plt.figimage(np.asarray(logo), xo=20, yo=-30)
            
            qr = Image.open(util.cross_project + '/Participant Plots/qr_code.png')
            qr = qr.resize((130, 160))
        ...
        #If ppl only have Kantaro results
            ...
            fig, axes = plt.subplots(2, 1,figsize=(8, 6), dpi=300, layout='tight', linewidth=10, height_ratios=[0.85, 0.15])

            def plot_raster(dates, color):
                for date in dates:
                    if pd.to_datetime(date) > pd.to_datetime('1.1.2020'):
                            plt.axvline(x=pd.to_datetime(date, errors='coerce'), color=color, lw=3, linestyle='--')
                plt.xlim(x_limits)
            ...
            logo = Image.open(util.cross_project + '/Participant Plots/simon_lab_logo.png')
            logo = logo.resize((200, 160))
            plt.figimage(np.asarray(logo), xo=5, yo=-30)
            
            qr = Image.open(util.cross_project + '/Participant Plots/qr_code.png')
            qr = qr.resize((115, 150))
